  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hovered ? `0 0 32px ${p.color}22` : "none",
        transform: hovered ? (side === "left" ? "translateX(-6px)" : "translateX(6px)") : "translateX(0)",
        position: "relative",
        overflow: "hidden",
        cursor: "default",
      }}
    >
      ...
      <p style={{ color: "var(--muted)", fontSize: 13, lineHeight: 1.7, marginBottom: 14 }}>{p.desc}</p>

      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: hovered ? 16 : 0, transition: "margin 0.3s" }}>
        {p.stack.map(s => (
      ...
      <div style={{ display: "flex", gap: 10, maxHeight: hovered ? 50 : 0, overflow: "hidden", opacity: hovered ? 1 : 0, transition: "max-height 0.35s ease, opacity 0.3s ease" }}>
        <a href={p.github} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 16px", borderRadius: 8, fontSize: 12, fontWeight: 600, background: "rgba(255,255,255,0.08)", border: "1px solid var(--border)", color: "var(--text)", textDecoration: "none", transition: "background 0.2s" }}
        ...
        <a href={p.demo} target="_blank" rel="noreferrer" style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 16px", borderRadius: 8, fontSize: 12, fontWeight: 600, background: `${p.color}22`, border: `1px solid ${p.color}55`, color: p.color, textDecoration: "none", transition: "background 0.2s" }}
